...
sequenceDiagram
    ...
    GW->>Auth: auth.register (create auth token)
    Auth->>DB: Store auth credentials
    Auth-->>GW: Auth token created
    GW-->>C: Registration success + JWT

    Note over C,DB: User Login Flow
    C->>GW: POST /auth/login
    GW->>Auth: auth.login
    Auth->>User: user.validate_credentials
    User->>DB: Verify user exists
    User-->>Auth: User validation result
    Auth->>Redis: Store session data
    Auth-->>GW: JWT + Session ID
    GW-->>C: Login success + JWT

    Note over C,DB: Protected User Operations
    C->>GW: GET /users/profile (with JWT)
    GW->>GW: JWT Guard validation
    GW->>Redis: Check session validity
    Redis-->>GW: Session valid
    GW->>User: user.get_profile
    User->>DB: Fetch user data
    User-->>GW: User profile data
    GW-->>C: User profile response

    Note over C,DB: User Update Flow
    C->>GW: PUT /users/profile (with JWT)
    GW->>GW: JWT Guard validation
    GW->>User: user.update_profile
    User->>DB: Update user record
    User-->>GW: Updated user data
    GW->>Redis: Update session cache
    GW-->>C: Update success response
```
